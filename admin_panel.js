...
async function setAvailability(slots) {
    ...
    const actionName = slots === 0 ? "CERRAR el día" : (slots === 1 ? "reducir a 1 Barbero" : "restablecer a Normal");
    if (!confirm(`¿Estás seguro de que quieres ${actionName} para el ${formatDateDisplay(currentDate)}?`)) {
    ...
    try {
        // 2. Verificar conflictos si reducimos capacidad (Cerrado o 1 Barbero)
        if (slots < 2) {
            ...
            if (slots === 0) {
                // Si cerramos, cancelamos TODAS las citas
                appointmentsToCancel = existingAppointments;
            } else if (slots === 1) {
                // Si reducimos a 1, necesitamos cancelar el exceso de citas por hora
                // Agrupamos por hora
                const appointmentsByTime = {};
                existingAppointments.forEach(appt => {
                    if (!appointmentsByTime[appt.time]) appointmentsByTime[appt.time] = [];
                    appointmentsByTime[appt.time].push(appt);
                });

                // Para cada hora, si hay más de 1 cita, cancelamos las sobrantes (las últimas en reservar)
                // Asumimos que el orden devuelto por supabase respeta inserción o podemos ordenar por created_at si estuviera disponible, 
                // pero aquí usaremos el orden del array.
                for (const time in appointmentsByTime) {
                    const appts = appointmentsByTime[time];
                    if (appts.length > 1) {
                        // Mantenemos la primera, cancelamos el resto
                        // (appts.slice(1) devuelve desde el índice 1 hasta el final)
                        appointmentsToCancel = [...appointmentsToCancel, ...appts.slice(1)];
                    }
                }
            }

            // 3. Procesar cancelaciones automáticas
            if (appointmentsToCancel.length > 0) {
                const confirmCancel = confirm(`⚠️ ATENCIÓN: Esta acción entrará en conflicto con ${appointmentsToCancel.length} cita(s) existente(s).\n\nSe enviarán correos de cancelación automáticamente y se eliminarán las citas.\n\n¿Deseas continuar?`);
                ...
                // Mostrar loading o notificar
                alert(`Procesando ${appointmentsToCancel.length} cancelaciones... Por favor espera.`);

                for (const appt of appointmentsToCancel) {
                    await cancelAppointmentWithoutConfirmation(appt);
                }
            }
        }

        // 4. Guardar la nueva disponibilidad
        if (slots === 2) {
        ...
        loadAppointments(); // Revert UI on error
    }
}

// Helper function to cancel without asking for confirmation again (used in batch)
async function cancelAppointmentWithoutConfirmation(appt) {
    try {
        // Send cancellation email
        const templateParams = {
            to_email: appt.email,
            to_name: appt.name,
            appointment_date: appt.date_string,
            appointment_time: appt.time,
            message: "Lamentamos informarle que su cita ha sido cancelada debido a un cambio en el horario de la barbería. Por favor, le invitamos a reservar una nueva hora a través de nuestra web. Disculpe las molestias."
        };

        try {
            await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_CANCEL_TEMPLATE_ID, templateParams);
            console.log(`Cancellation email sent to ${appt.email}`);
        } catch (emailError) {
            console.error(`Error sending cancellation email to ${appt.email}:`, emailError);
        }
        ...
        console.error(`Error canceling appointment for ${appt.name}:`, error);
    }
}
